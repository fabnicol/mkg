\hypertarget{group__mk_file_system}{}\doxysection{Create Gentoo linux filesystem on VM disk}
\label{group__mk_file_system}\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!adjust\_environment@{adjust\_environment}}
\index{adjust\_environment@{adjust\_environment}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
\item Oneshot emerge of {\bfseries{cmake}} and {\bfseries{lz4}}, prerequisites to {\ttfamily  world } update. \item Update {\ttfamily  world }. Log into emerge.\+build. Exit on error. \item Set keymaps, localization and time \begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000003}{Todo}}]Add more regional options by parametrization of commandline \end{DoxyRefDesc}

\begin{DoxyRetVals}{Return values}
{\em -\/1} & on error at {\ttfamily emerge} stage. \\
\hline
\end{DoxyRetVals}
\end{DoxyItemize}


Definition at line 40 of file mkvm\+\_\+chroot.\+sh.
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!build\_kernel@{build\_kernel}}
\index{build\_kernel@{build\_kernel}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
Definition at line 229 of file mkvm\+\_\+chroot.\+sh.
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!finalize@{finalize}}
\index{finalize@{finalize}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
Definition at line 319 of file mkvm.\+sh.
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!global\_config@{global\_config}}
\index{global\_config@{global\_config}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
\begin{DoxyItemize}
\item Cleanup log, distfiles (for deployment), kernel build sources and objects \item Log this into {\bfseries{log\+\_\+uninstall\+\_\+software}} \item Update {\bfseries{eix}} cache. Sets display for {\bfseries{display-\/manager}}. \item Add services\+: {\bfseries{sysklog, cronie, display-\/manager, dbus, elogind}} \item Substitute {\bfseries{Network\+Manager}} to temporary networking setup. \item Adjust group and {\bfseries{sudo}} settings for non-\/root user and {\bfseries{sddm}} \item Install {\bfseries{grub}} in EFI mode. \item Set passwords for root and non-\/root user. \begin{DoxyWarning}{Warning}
Fix {\bfseries{sddm}} startup keyboard issue using {\ttfamily  setxkbmap} 
\end{DoxyWarning}
\end{DoxyItemize}


Definition at line 375 of file mkvm\+\_\+chroot.\+sh.
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!install\_software@{install\_software}}
\index{install\_software@{install\_software}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
\begin{DoxyItemize}
\item Emerge list of ebuilds on top of stage3 and system utils already merged. \item Optionaly download and build RStudio 
\begin{DoxyRetVals}{Return values}
{\em -\/1} & on building errors \\
\hline
\end{DoxyRetVals}
\begin{DoxyRefDesc}{Todo}
\item[\mbox{\hyperlink{todo__todo000004}{Todo}}]Add a script to build R dependencies\end{DoxyRefDesc}
\end{DoxyItemize}


Definition at line 290 of file mkvm\+\_\+chroot.\+sh.
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!install\_stage3@{install\_stage3}}
\index{install\_stage3@{install\_stage3}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!partition@{partition}}
\index{partition@{partition}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
...
\index{Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}!setup\_network@{setup\_network}}
\index{setup\_network@{setup\_network}!Create Gentoo linux filesystem on VM disk@{Create Gentoo linux filesystem on VM disk}}
